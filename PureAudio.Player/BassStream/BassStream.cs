...
namespace MediaPortal.Plugins.PureAudio
{
  ...
  public partial class BassStream : IDisposable
  {
    ...
    /// <summary>
    /// Gets or sets the volume in dB (-100dB - 0dB)
    /// </summary>
    public float Volume
    {
      get
    ...
    /// <summary>
    /// Positions the stream at the start of the next frame. Only for encoded multichannel data.
    /// </summary>
    ...
    private void Initialize()
    {
      ...
      if (
          _Info.ctype != BASSChannelType.BASS_CTYPE_STREAM &&
          _Info.ctype != BASSChannelType.BASS_CTYPE_STREAM_MIXER)
      {
        _Length = GetLength();

        _StreamContentType = GetStreamContentType();
        Log.Info("BassStream: Stream info: {0} channels, {1}Hz, {2}bit, {3}, {4}",
            _Info.chans,
            _Info.freq,
            _Info.origres,
            _StreamContentType,
      ...
      {
        Log.Debug("BassStream: Stream info: {0} channels, {1}Hz, {2}bit, {3}",
            _Info.chans,
            _Info.freq,
            _Info.Is8bit ? 8 : _Info.Is32bit ? 32 : 16,
            _Info.ctype);
      }
    }
    ...
    /// <summary>
    /// Sets the current stream volume in dB (-100dB - 0dB).
    /// </summary>
    /// <param name="volume"></param>
    private void SetVolume(float volume)
    {
      float attributeValue = (100f + volume) / 100f;
      if (!Bass.BASS_ChannelSetAttribute(_Handle, BASSAttribute.BASS_ATTRIB_VOL, attributeValue))
        throw new BassLibraryException("BASS_ChannelSetAttribute");
    }

    /// <summary>
    /// Gets the current stream volume in dB (-100dB - 0dB).
    /// </summary>
    /// <returns></returns>
    private float GetVolume()
    {
      float attributeValue = 0.0f;
      if (!Bass.BASS_ChannelGetAttribute(_Handle, BASSAttribute.BASS_ATTRIB_VOL, ref attributeValue))
        throw new BassLibraryException("BASS_ChannelGetAttribute");

      return ((attributeValue * 100f) - 100f);
    }
